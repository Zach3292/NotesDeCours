...
On peut décrire une tâche selon sa fréquence d'exécution et son temps d'exécution.
...
#### Périodique
Une tâche périodique est une tâche qui doit être exécuté selon un temps de cycle fixe, par exemple, à toutes les secondes. Il est important de prendre en compte le temps que prend une tâche périodique à s'exécuter pour ne pas qu'elle occupe tout le temps de processeur.
...
#### Sporadique
Une tâche sporadique est comme une tâche apériodique mais qui requière habituellement un *hard ou firm real-time*. Les tâches sporadiques par leur nature peuvent occuper tout le temps de processeur pendant un moment. Il faut alors décider de la priorité des tâches. Lorsque ces tâches sont prêtes à rouler, elles sont traitées comme des tâches périodiques. Pour pouvoir exécuter les tâches sporadiques, il est nécessaire qu'il y ait un interval minimal entre celle-ci pour laisser le temps au processeur de les gérer.
